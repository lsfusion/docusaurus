...
|Name|Type|Description|Default|
|---|---|---|---|
|`host`, `port`, `exportName`|`string`, `int`, `string`|Connection settings for the application server. Must match the [access parameters](#accessapp-broken) for the application server.<ul><li>`host` - application server address</li><li>`port` - port of the application server. Must match the parameter `rmi.port`</li><li>`exportName` - name of the application server. Must match the parameter `rmi.exportName`</li></ul>|`localhost`, `7652`, `default`|
