...
The key features that implement the extension technique are the [`ABSTRACT` operator](ABSTRACT_operator_action_.md), for declaring an abstract action, and the [`ACTION+` statement](ACTION+_instruction.md), for adding an implementation to it.
